...
export default function CreateRoomScreen() {
  ...
  const onCreate = async () => {
    if (!title.trim()) return Alert.alert('Missing', 'Enter a room title');
    setLoading(true);
    try {
      const room = await createRoom(title.trim());
      router.replace({ pathname: '/chat', params: { roomId: (room as any).id } });
    } catch (e: any) {
  ...
  return (
    <View style={{ flex: 1, paddingTop: insets.top, padding: 16 }}>
      <Text style={{ fontSize: 24, fontWeight: '600', marginBottom: 16 }}>Create Room</Text>
      <Text>Title</Text>
      <TextInput
        value={title}
